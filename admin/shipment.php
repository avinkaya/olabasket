                                ...
                                <div class="table-responsive">
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        ...
                                        <tbody>
                                            <?php
                                                                    $fetchData = array();
$fetchData = $db->rawQuery("SELECT CM.* FROM ".CATEGORY." CM WHERE CM.is_deleted = '0' ORDER BY CM.category_id DESC");
if(count($fetchData) > 0){
    for($i=0;$i<count($fetchData);$i++){
        ...
        $category_slug = $fetchData[$i]['category_slug'];
        $category_image = ADMIN_PANEL.$fetchData[$i]['category_image'];
        $category_remark = $fetchData[$i]['category_remark'];
        $category_index = $fetchData[$i]['category_index'];
        ...
        ?>
                                            <tr>
                                                ...
                                                <td><?= $category_name; ?></td>
                                                <td class="text-center"><img width="50" class="rounded-circle" src="<?= $category_image; ?>" alt=""></td>
                                                <td><?= $category_remark; ?></td>
                                                ...
                                                <td class="text-center"><?= $category_index; ?></td>
                                                <td class="text-center">
                                                    <?php
                                                if($is_active == "1"){
                                                    ?>
							...
							<a href="javascript:void(0)" class="btn btn-danger shadow btn-xs sharp" 
                                                           title="Remove" key="category_id" val="<?= $category_id; ?>" 
                                                           function_name="removeCategoryData" 
...
?>
           ...
           <!-- Modal -->
                                        ...
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content modal-sm">
                                                ...
                                                <div class="modal-body">
                                                    <input type="hidden" name="case" id="case" value="addEditCategoryData">
                                    <input type="hidden" name="category_id" id="category_id">
                                            ...
                                            <div class="form-group col-md-12">
                                                <label>Slug</label>
                                                <input type="text" class="form-control" id="category_slug" name="category_slug" placeholder="Enter Slug" required="">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>Remark</label>
                                                <textarea class="form-control" id="category_remark" name="category_remark" placeholder="Enter Remark"></textarea>
                                            </div>
                                        </div>
                                    <div class="form-row">
                                            ...
                                            <div class="form-group col-md-4 text-center">
                                                <input type="hidden" id="image" name="category_image">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-4">
                                                <label>Index</label>
                                                <input type="number" class="form-control" id="category_index" name="category_index" placeholder="Enter Index">
                                            </div>
                                        </div>
                                                </div>
    ...
    <script>
       ...
       function editData(ele){
           $("#category_id").val($(ele).attr("category_id"));
           $("#category_name").val($(ele).attr("category_name"));
           $("#category_slug").val($(ele).attr("category_slug"));
           $("#category_remark").val($(ele).attr("category_remark"));
           $("#category_index").val($(ele).attr("category_index"));
           $("#imageUrl").attr("src",$(ele).attr("category_image"));
           $("#is_active").val($(ele).attr("is_active")).trigger("change");
           $("#popUpModalTitle").html("Edit <?= $title; ?> - "+ $(ele).attr("category_name"));
...
function convertToSlug(Text) {
  return Text.toLowerCase()
        ...
        $(document).ready(function() {
       $('#category_name').on('change blur', function() {
            var category_name = this.value;
            if(category_name != ""){
            $('#category_slug').val(convertToSlug(category_name));
        }
});
   });
    </script>
